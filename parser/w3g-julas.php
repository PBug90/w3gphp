...
class replay
{
    public $fp;
    public $data;
    public $leave_unknown;
    public $continue_game;
    public $referees;
    public $time;
    public $apmTimeIndex;
    public $apmCurrentTimeFrame;
    public $pause;
    public $leaves;
    public $errors;
    public $header;
    ...
    public function replay($filename, $parse_actions=true, $parse_chat=true)
    {
        ...
        $start = strrpos($content, "Warcraft III recorded game");
        if ($start === false) {
            exit(1);
        }

        fseek($this->fp, $start);
        $this->read_offset = $start;
        $this->parseheader();
        $this->parsedata();
        $this->cleanup();
    ...
    {
        fseek($this->fp, $this->header['header_size']+$this->read_offset);
            ...
            switch ($block_id) {
                ...
                case 0x1F:
                    ...
                    if (!$this->pause) {
                        ...
                        $this->apmCurrentTimeFrame+= $temp['time_inc'];
                        if ($this->apmCurrentTimeFrame >= $this->apmTimeSampleInterval) {
                            $this->apmTimeIndex +=1;
                            $this->apmCurrentTimeFrame = 0;
                        }
                    }
    ...
    public function parseactions($actionblock, $data_length)
    {
        $block_length = 0;
        $action = 0;
            ...
            while ($n < $block_length) {
                ...
                switch ($action) {
                    ...
                    case 0x6B:
                       ...
                       // echo(sprintf('Hex von n: %02X<br>',ord($actionblock[$n])));
                        ...
                        if (trim($value) === "MMD.Dat") {
                            ...
                            } elseif ($value[0] === "DefVarP") {
                                $this->arm["detected_events"][] = $value;
                            } elseif ($value[0] === "FlagP") {
                                $pid= $value[1];
                                $this->arm["player"][$pid]["flags"][]=$value[2];
                            } elseif ($value[0] === "VarP") {
                                $pid= $value[1];
                                $this->arm["player"][$pid]["vars"][$value[2]]=$value[4];
                            }
                        }
                        $n+=4;
                       // echo(sprintf('Hex von n: %02X<br>',ord($actionblock[$n])));
                    ...
                    case 0x10:
                        ...
                        $this->players[$player_id]['actions'][$this->apmTimeIndex]++;
                    ...
                    case 0x12:
                        $this->players[$player_id]['actions']++;
                        $this->players[$player_id]['actions_timed'][$this->apmTimeIndex]++;
                        if ($this->header['major_v'] >= 13) {
                            $n++; // ability flag
                        }
